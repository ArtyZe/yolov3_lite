...
convolutional_layer parse_convolutional(list *options, size_params params, int count)
{
    ...
    int weight_quant_flag = option_find_int_quiet(options, "weight_quant", 0);
    int activ_quant_flag = option_find_int_quiet(options, "activ_quant", 0);
#ifdef QUANTIZATION_GOOGLE
    int quantized = option_find_int_quiet(options, "quantized", 1);
#else
    int quantized = (option_find_int_quiet(options, "quantized", 1) && params.quantized);
#endif

    convolutional_layer layer = make_convolutional_layer(batch,h,w,c,n,groups,size,stride,padding,activation, batch_normalize, 
                                                         binary, xnor, count, quantized, weight_quant_flag, activ_quant_flag);
...
maxpool_layer parse_maxpool(list *options, size_params params)
{
    ...
    maxpool_layer layer = make_maxpool_layer(batch,h,w,c,size,stride,padding,activ_quant_flag);
    return layer;
}
...
void parse_net_options(list *options, network *net)
{
    ...
    char *ss = option_find_str(options, "output_scale", 0);
    char *zz = option_find_str(options, "output_zero_point", 0);
    if (a) {
        int len = strlen(a);
        int n = 1;
        int i;
    ...
    if (ss) {
        ...
        net->output_scale = (float *)calloc(n, sizeof(float));
        for (i = 0; i < n; ++i) {
            float coef_b = atof(ss);
            net->output_scale[i] = coef_b;
            ss = strchr(ss, ',') + 1;
        }
    }
    if (zz) {
        ...
        net->output_zero_point = (uint8_t *)calloc(n, sizeof(uint8_t));
        for (i = 0; i < n; ++i) {
            float coef_c = atof(zz);
            net->output_zero_point[i] = coef_c;
            zz = strchr(zz, ',') + 1;
        }
    }
...
#endif
#ifdef QUANTIZATION_GOOGLE
    fwrite(l.activ_data_int8_scales, sizeof(float), 1, fp);
    fwrite(l.activ_data_int8_zero_point, sizeof(uint8_t), 1, fp);
    fwrite(l.weight_data_int8_scales, sizeof(float), 1, fp);
    fwrite(l.weight_data_int8_zero_point, sizeof(uint8_t), 1, fp);
    fwrite(l.weights_quant, sizeof(uint8_t), l.c*l.n*l.size*l.size, fp);
    printf("weight quant value is %d\n", l.weights_quant[20]);
#endif
    fwrite(l.weights, sizeof(float), num, fp);
}

void save_maxpool_weights(layer l, FILE *fp)
{
    fwrite(l.activ_data_int8_scales, sizeof(float), 1, fp);
    fwrite(l.activ_data_int8_zero_point, sizeof(uint8_t), 1, fp);
...
#endif
#ifdef QUANTIZATION_GOOGLE
    fread(l.activ_data_int8_scales, sizeof(float), 1, fp);
    fread(l.activ_data_int8_zero_point, sizeof(uint8_t), 1, fp);
    fread(l.weight_data_int8_scales, sizeof(float), 1, fp);
    fread(l.weight_data_int8_zero_point, sizeof(uint8_t), 1, fp);
    fread(l.weights_quant, sizeof(uint8_t), l.c*l.n*l.size*l.size, fp);
#endif
...
void load_maxpool_weights(layer l, FILE *fp)
{
    fread(l.activ_data_int8_scales, sizeof(float), 1, fp);
    fread(l.activ_data_int8_zero_point, sizeof(uint8_t), 1, fp);
}
