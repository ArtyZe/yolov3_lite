...
maxpool_layer make_maxpool_layer(int batch, int h, int w, int c, int size, int stride, int padding, int activ_quant_flag)
{
    maxpool_layer l;
    l.type = MAXPOOL;
    init_layer(l);
    ...
    l.input_quant = (uint8_t*)calloc(output_size, sizeof(uint8_t));
    #endif
    #ifdef GPU
        #ifdef QUANTIZATION_GOOGLE
