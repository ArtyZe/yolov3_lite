...
void delta_yolo_class(float *output, float *delta, int index, int cls, int classes, int stride, float *avg_cat)
{
    ...
    if (delta[index]){
        ...
        if(avg_cat) *avg_cat += output[index + stride*cls];
        return;
    }
    for(n = 0; n < classes; ++n){
        delta[index + stride*n] = ((n == cls)?1 : 0) - output[index + stride*n];
        if(n == cls && avg_cat) *avg_cat += output[index + stride*n];
    }
}
    ...
    for (b = 0; b < l.batch; ++b) {
            ...
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    if (best_iou > l.truth_thresh) {
                        ...
                        int cls = net.truth[best_t*(4 + 1) + b*l.truths + 4];
                        if (l.map) cls = l.map[cls];
                        int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 4 + 1);
                        delta_yolo_class(l.output, l.delta, class_index, cls, l.classes, l.w*l.h, 0);
        ...
        for(t = 0; t < l.max_boxes; ++t){
            ...
            if(mask_n >= 0){
                ...
                int cls = net.truth[t*(4 + 1) + b*l.truths + 4];
                if (l.map) cls = l.map[cls];
                int class_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 4 + 1);
                delta_yolo_class(l.output, l.delta, class_index, cls, l.classes, l.w*l.h, &avg_cat);
