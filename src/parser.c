...
convolutional_layer parse_convolutional(list *options, size_params params, int count)
{
    ...
    convolutional_layer layer = make_convolutional_layer(batch,h,w,c,n,groups,size,stride,padding,activation, batch_normalize, binary, xnor, count);
...
void save_convolutional_weights(layer l, FILE *fp)
{
    ...
    if (l.batch_normalize){
		...
		for(i=0; i<num; i++){
			if(l.weights[i]==0){
	    		fwrite(l.weights+i, sizeof(float), 1, fp);
	    }else{
	    		fwrite(l.weights+i, sizeof(float), 1, fp);
	    }
	  }
